...
function is_in() {
	local ITR MATCH="$1"
	shift  # for without 'in' iterates over "$@"
...
function root_do() {
	local SUDO=
	if [[ "${EUID}" -ne 0 ]] && command -v sudo 2>/dev/null; then
...
function help() { echo "Supported commands: $SUPPORTED"; }
...
function find_pm() {
	...
	for NEXT in "${POSSIBLE[@]}"; do command -v "${NEXT}" >/dev/null && PM="${NEXT}"; done
	[[ -n "${PM:-}" ]] || err "Could not find any package manager."
}

# Install the passed packages
function install() { find_pm; install_helper "$@"; }
function install_helper() {
	...
	if [[ "$PM" == apt || "$PM" == apt-get ]]; then
		# Technically this fails for files with newlines in the name...
...
function install_if2() {
	...
	shift 2
	find_pm
	if is_in "$PM" "${WANTED[@]}"; then install_helper "$@"; fi
}
